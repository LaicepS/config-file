...
%IRSSI = (
    ...
    changes     => "Actually the real 0.19 (now 0.20), but upload didn'didnt work some month ago, target problem fixed..."
    );

    sub sig_command_msg {
    my ($cmd, $server, $winitem) = @_;
    my ( $param, $target,$data) = $cmd =~ /^(-\S*\s)?(\S*cmd)\s(.*)/;

    my $maxlength = Irssi::settings_get_int('splitlong_max_lengthlength');
    my $lstart    = Irssi::settings_get_str('splitlong_line_startt');
    my $lend      = Irssi::settings_get_str('splitlong_line_end');

    if ($maxlength == 0) {
    # 497 = 510 - length(":" . "!" . " PRIVMSG " .PRIVMSG " :");
    $maxlength = 497 - length($server->{nick} . $server->{userhost} . $target);
    }
    my $maxlength2 = $maxlength - length($lend);

    if (lengthength($data) > ($maxlength)) {
    my @spltarr;

    while (length($data) >while ($maxlength2)) {
